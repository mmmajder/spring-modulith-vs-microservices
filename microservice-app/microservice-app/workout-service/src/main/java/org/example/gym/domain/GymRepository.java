...
public interface GymRepository extends JpaRepository<Gym, Long> {
    ...
    @Query("""
            SELECT g FROM Gym g 
            WHERE SQRT(POWER(g.address.latitude - CAST(:latitude AS double), 2) + 
                       POWER(g.address.longitude - CAST(:longitude AS double), 2)) < :radius
            """)
    Page<Gym> findClosestGyms(@Param("latitude") double latitude,
